// Navbar.jsx
// Top bar — toggle button inside normal flow (no overlap possible).

import { useState, useRef, useEffect } from "react";
import { Search, Bell, X, ChevronDown, Menu } from "lucide-react";
...
function Navbar({
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-theme bg-theme-secondary/95 backdrop-blur">
      <div className="flex h-[64px] items-center justify-between gap-2 px-3 sm:h-[72px] sm:gap-4 sm:px-4 md:px-6 lg:px-8">
        {/* ============ LEFT: Toggle + Search ============ */}
        <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
          {/* Toggle Button — inside flow, no overlap */}
          <button
            type="button"
            onClick={onMenuClick}
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-theme bg-theme-tertiary text-theme-secondary transition hover:border-indigo-300 hover:text-indigo-600 active:scale-95 sm:h-10 sm:w-10"
          ...
          {/* Desktop Search */}
          <div className="hidden w-full max-w-md items-center gap-2 rounded-xl border border-theme bg-theme-tertiary px-3.5 py-2.5 transition focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-100 sm:flex">
          ...
          <div className="flex min-w-0 flex-1 items-center sm:hidden">
            {!showMobileSearch ? (
              <h2 className="truncate text-sm font-bold text-theme-primary">
                SmartEvent
              </h2>
            ) : (
              <div className="flex flex-1 items-center gap-2 rounded-xl border border-theme bg-theme-tertiary px-3 py-2 transition focus-within:border-indigo-400">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-xl text-theme-muted transition hover:bg-theme-hover hover:text-theme-primary sm:hidden"
            aria-label="Toggle search"
          >
            <Search size={18} />
          </button>

          {/* BB-8 Theme Toggle — desktop */}
          ...
          <div className="relative" ref={notificationRef}>
            <button
              ...
              className="relative flex h-9 w-9 items-center justify-center rounded-xl text-theme-muted transition hover:bg-theme-hover hover:text-theme-primary sm:h-10 sm:w-10"
          ...
          <div className="relative" ref={profileRef}>
            <button
              ...
              className="flex items-center gap-2 rounded-xl px-1 py-1 transition hover:bg-theme-hover sm:px-1.5"
            >
              ...
              <ChevronDown size={14} className="hidden text-theme-muted lg:block" />
            </button>
